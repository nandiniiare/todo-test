...
describe("Tests for functions in todo.js", function () {
 ...
 test("Test to add a todo", async () => {
  ...
  expect(todo.dueDate).toEqual(getJSDate(3));
 ...
 test("Test to retrieve overdue items", async () => {
  ...
  expect(overdueItems[0].dueDate).toBeLessThan(new Date()); // Due date is in the past
});
...
test("Test to retrieve due today items", async () => {
  ...
  expect(dueTodayItems[0].dueDate.toDateString()).toBe(new Date().toDateString());
});
...
test("Test to retrieve due later items", async () => {
  ...
  expect(dueLaterItems[0].dueDate).toBeGreaterThan(new Date()); // Due date is in the future
});
